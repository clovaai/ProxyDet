...
'''
    ...
    original source: https://github.com/facebookresearch/Detic/blob/main/train_net.py
'''
import os
...
from proxydet.data.custom_build_augmentation import build_custom_augmentation
from proxydet.data.custom_dataset_dataloader import  build_custom_train_loader
from proxydet.data.custom_dataset_mapper import CustomDatasetMapper, DetrDatasetMapper
from proxydet.custom_solver import build_custom_optimizer
from proxydet.evaluation.oideval import OIDEvaluator
from proxydet.evaluation.custom_coco_eval import CustomCOCOEvaluator
from proxydet.modeling.utils import reset_cls_test
...
def do_train(cfg, model, resume=False):
    ...
    with EventStorage(start_iter) as storage:
        ...
        for data, iteration in zip(data_loader, range(start_iter, max_iter)):
            ...
            if (cfg.EVAL_START > 0 and iteration >= cfg.EVAL_START) or (cfg.EVAL_START == 0):
                if (cfg.TEST.EVAL_PERIOD > 0
                    and iteration % cfg.TEST.EVAL_PERIOD == 0
                    and iteration != max_iter):
                    logger.info("Starting evaluation from iteration {}".format(iteration))
                    do_test(cfg, model)
                    comm.synchronize()
                    logger.info("Saving checkpoint from iteration {}".format(iteration))
                    periodic_checkpointer.save("{}_{:07d}".format("model", iteration))
...
def setup(args):
    ...
    setup_logger(output=cfg.OUTPUT_DIR, \
        distributed_rank=comm.get_rank(), name="proxydet")
    return cfg
