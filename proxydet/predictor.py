...
class VisualizationDemo(object):
    def __init__(self, cfg, args, 
        ...
        if args.zeroshot_weight_path is not None:
            trained_zs_classifier = torch.tensor(
                np.load(args.zeroshot_weight_path), 
                dtype=torch.float32
            ).permute(1, 0).contiguous().to(cfg.MODEL.DEVICE) # D x C
            trained_zs_classifier = F.normalize(trained_zs_classifier, p=2, dim=0)

            base_cat_indices = self.predictor.model.roi_heads.base_cat_mask.nonzero().squeeze(-1)
            # for debug purpose
            # from .cat_names import lvis_cat_names
            # base_cat_names = [lvis_cat_names[el] for el in base_cat_indices]
            # novel_cat_names = list(set(lvis_cat_names) - set(base_cat_names))

            # L2 normalized
            custom_classifier = self.predictor.model.roi_heads.box_predictor[0].cls_score.zs_weight
            base_cat_sim = custom_classifier.T[:-1, :] @ trained_zs_classifier[:, base_cat_indices]
            # print(base_cat_sim.max(dim=1)[0], [base_cat_names[el] for el in base_cat_sim.argmax(dim=1)])
                
            # reset base cat mask
            new_base_cat_mask = torch.cat([base_cat_sim.max(dim=1)[0] > args.base_cat_threshold, base_cat_sim.new_zeros((1))], dim=0) # for bg
